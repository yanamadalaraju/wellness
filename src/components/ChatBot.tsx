...
const ChatBot: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {/* Chat Window */}
      {isOpen && (
        <div className="w-80 h-96 bg-white rounded-lg shadow-xl mb-4 flex flex-col overflow-hidden border border-[#67C090]">
          {/* Header */}
          <div className="bg-[#67C090] text-white p-4 flex justify-between items-center">
            <h3 className="font-semibold">Nowal Assistant</h3>
            <button
              onClick={() => setIsOpen(false)}
                ...
                <div
                  className={`max-w-xs px-3 py-2 rounded-lg ${
                    message.sender === 'user'
                      ...
                      : 'bg-white text-gray-800 border border-[#67C090]'
                  }`}
                >
                  {message.text}
                </div>
          ...
          <div className="p-4 border-t border-[#67C090]">
            <div className="flex space-x-2">
              <input
                ...
                onKeyPress={(e) => e.key === 'Enter' && handleSend()}
                placeholder="Type your message..."
                className="flex-1 px-3 py-2 border border-[#67C090] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#67C090] bg-white"
              />
              <button
                onClick={handleSend}
                className="bg-[#67C090] text-white px-3 py-2 rounded-lg hover:bg-[#5AA87E] transition-colors"
              >
                <Send className="w-4 h-4" />
              </button>
            </div>
      ...
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center justify-center w-14 h-14 bg-[#67C090] hover:bg-[#5AA87E] rounded-full shadow-lg hover:shadow-xl transition-all duration-300 text-white"
      >
        <MessageCircle className="w-6 h-6" />
      </button>
    </div>
